...
extension DMLicenseSelectionVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func updateCellForPhotoCell(cell: PhotoCell, indexPath: IndexPath) {
        ...
        if stateBoardImage == nil {
            ...
            cell.stateBoardPhotoButton.setImage(stateBoardImage!, for: .normal)
        }
        cell.selectionStyle = .none
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        if textField.tag == 0 {
            licenseArray?.replaceObject(at: 0, with: textField.text!)
        } else {
            licenseArray?.replaceObject(at: 1, with: textField.text!)
        }
    }
